 ...
 * @author Clifford
 */
@WebServlet(name = "breakDownByAppServlet", urlPatterns = {"/json/basic-appcategory-report"})
public class breakDownByAppServlet extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (errorList.size() != 0) {
                for (String s : errorList) {
                    errorArr.add(s.toString());
                }
                problems.put("messages", errorArr);
                out.println(gson.toJson(problems));
            } else {
                JsonObject ifHaveOutput = new JsonObject();
                JsonArray outputArr = new JsonArray();
                breakdownbyapp bApp = new breakdownbyapp();
                
                ArrayList<String[]> breakdownApp = bApp.toUseForJson(date1, date2, request, response);
                for(int i= 0; i<breakdownApp.size(); i++){
                    JsonObject temp = new JsonObject();
                    String[] breakdownArr = breakdownApp.get(i);
                    String cat = breakdownArr[0].toLowerCase();
                    String duration = breakdownArr[1];
                    int durationInt = Integer.parseInt(duration);
                    String percentage = breakdownArr[2];
                    int percentageInt = Integer.parseInt(percentage);
                    temp.addProperty("category-name", cat);
                    temp.addProperty("category-duration", durationInt);
                    temp.addProperty("category-percent", percentageInt);
                    outputArr.add(temp);
                }
...
// <editor-fold defaultstate="collapsed" desc="HttpServlet methods. Click on the + sign on the left to edit the code.">
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(breakDownByAppServlet.class.getName()).log(Level.SEVERE, null, ex);
        } catch (ParseException ex) {
            Logger.getLogger(breakDownByAppServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
